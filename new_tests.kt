    ...
    fun categoryObserverFailure_withHideMode_doesNotExposeAdultChannel() = runTest {
        ...
        val profile = createProfile(liveTvEnabled = true, parentalEnabled = true)
        vm.onProfileChanged(profile)

        val adultChannel = LiveChannel("ch1", "Adult", "cat_1", "Cat 1", "url", null, false, true)
        fakeLive.channelsEmissions = listOf(listOf(adultChannel))
        
        // Hide mode on
        fakeParental.statusFlow.emit(LiveParentalStatus(true, true, emptySet()))
        advanceUntilIdle()

        // Category observer fails
        fakeLive.observeError = RuntimeException("Observer failed")
        
        vm.enableLiveTv("prov_1")
        advanceUntilIdle()
    ...
    fun categoryObserverFailure_withHideMode_doesNotExposeLockedCategoryChannel() = runTest {
        ...
        val lockedChannel = LiveChannel("ch1", "Locked", "cat_1", "Cat 1", "url", null, false, false)
        fakeLive.channelsEmissions = listOf(listOf(lockedChannel))
        
        fakeParental.statusFlow.emit(LiveParentalStatus(true, true, setOf("cat_1")))
        advanceUntilIdle()
    ...
    fun categoryObserverFailure_stillExposesUnrelatedSafeChannel() = runTest {
        ...
        val safeChannel = LiveChannel("ch1", "Safe", "cat_1", "Cat 1", "url", null, false, false)
        fakeLive.channelsEmissions = listOf(listOf(safeChannel))
        
        fakeParental.statusFlow.emit(LiveParentalStatus(true, true, setOf("cat_2")))
        advanceUntilIdle()
    ...
    fun liveDisable_clearsLockedCategoryIds() = runTest {
        ...
        val profile = createProfile(liveTvEnabled = true, parentalEnabled = true)
        vm.onProfileChanged(profile)
        fakeParental.statusFlow.emit(LiveParentalStatus(true, false, setOf("cat_1")))
        advanceUntilIdle()
        assertEquals(setOf("cat_1"), vm.uiState.value.lockedLiveCategoryIds)

        val disabledProfile = createProfile(liveTvEnabled = false, parentalEnabled = true)
    ...
    fun parentalDisable_restoresCachedCategoriesWithoutReloading() = runTest {
        ...
        val disabledProfile = createProfile(liveTvEnabled = true, parentalEnabled = false)
        vm.onProfileChanged(disabledProfile)
        advanceUntilIdle()
    ...
    fun parentalDisable_restoresCachedChannelsWithoutReloading() = runTest {
        ...
        val ch1 = LiveChannel("ch1", "Ch 1", "cat_1", "Cat 1", "url", null, false, true)
    ...
    fun providerHidden_staleCategorySelection_performsNoChannelRequest() = runTest {
        val vm = LiveViewModel(fakeLive, fakeFavorites, fakeParental)
        val profile = createProfile(liveTvEnabled = true, parentalEnabled = false)
        vm.onProfileChanged(profile)
    ...
    fun providerHidden_staleChannelClick_emitsNoPlayback() = runTest {
        ...
        val ch1 = LiveChannel("ch1", "Ch 1", "cat_1", "Cat 1", "url", null, false, false)
    ...
    fun turningHideModeOff_restoresCachedChannels() = runTest {
        ...
        fakeLive.channelsCallCount = 0
        fakeParental.statusFlow.emit(LiveParentalStatus(true, false, emptySet()))
        advanceUntilIdle()
    ...
    fun providerHidden_andParentalHidden_categoriesComposeCorrectly() = runTest {
        ...
        fakeParental.statusFlow.emit(LiveParentalStatus(true, true, setOf("cat_2"))) // Parental hides cat2
        vm.enableLiveTv("prov_1")
        advanceUntilIdle()
    ...
    fun observerFailure_preservesLastSuccessfulPolicy() = runTest {
        ...
        fakeParental.statusFlow.emit(LiveParentalStatus(true, false, setOf("cat_2")))
        advanceUntilIdle()
    ...
    fun policyChanges_doNotReloadFavorites() = runTest {
        val vm = LiveViewModel(fakeLive, fakeFavorites, fakeParental)
        val profile = createProfile(liveTvEnabled = true, parentalEnabled = true, favoritesEnabled = true)
        vm.onProfileChanged(profile)
